                        ...
                        <motion.div key="difficulty" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col items-center w-full">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full mb-8">
                                <AppButton
                                    ...
                                    className={`flex flex-col py-6 ${difficulty === 'easy' ? 'ring-4 ring-green-500 scale-105' : ''}`}
                                    onClick={() => { playSound('click'); setDifficulty('easy'); }}
                                >
                                    <span className="text-2xl mb-2">やさしい</span>
                                    <span className="text-sm font-normal">3〜4さい むけ</span>
                                </AppButton>

                                <AppButton
                                    color={difficulty === 'normal' ? 'blue' : 'gray'}
                                    className={`flex flex-col py-6 ${difficulty === 'normal' ? 'ring-4 ring-blue-500 scale-105' : ''}`}
                                    onClick={() => { playSound('click'); setDifficulty('normal'); }}
                                >
                                    <span className="text-2xl mb-2">ふつう</span>
                                    <span className="text-sm font-normal">5〜6さい むけ</span>
                                </AppButton>

                                <AppButton
                                    color={difficulty === 'hard' ? 'red' : 'gray'}
                                    className={`flex flex-col py-6 ${difficulty === 'hard' ? 'ring-4 ring-red-500 scale-105' : ''}`}
                                    onClick={() => { playSound('click'); setDifficulty('hard'); }}
                                >
                                    <span className="text-2xl mb-2">むずかしい</span>
                                    <span className="text-sm font-normal">しょうがくせい むけ</span>
                                </AppButton>
                            </div>
